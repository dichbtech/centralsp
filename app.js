...
let usuarioLogadoNick = '';
let nivelUsuarioGlobal = 'SUPERVISOR';
let acessosData = [];
let planilhaAcessos = {};
let membrosDataArray = [];
let dashboardEventosData = [];
let pontosExtrasMap = {};
...
auth.onAuthStateChanged((user) => {
    if (user) {
        usuarioLogadoEmail = user.email.toLowerCase();
        verificarAcessoBD(usuarioLogadoEmail);
    } else {
...
async function verificarAcessoBD(email) {
    try {
        ...
        if (userPlan && (userPlan.nivel.includes("LIDER") || userPlan.nivel.includes("VICE"))) {
            ...
            usuarioLogadoNick = userPlan.nick;
        } else if (userManual) {
            autorizado = true;
            nivelUsuarioGlobal = userManual.nivel;
            usuarioLogadoNick = userManual.nick;
        } else if (userPlan) {
            autorizado = true;
            nivelUsuarioGlobal = userPlan.nivel;
            usuarioLogadoNick = userPlan.nick;
...
function escutarMilitaresEstrelas() {
...
function renderTabelaEstrelas() {
    ...
    lista.forEach(dados => {
        let cargo = cargosMap[dados.nome] || '';
...
function registrarLogEstrela(bene, acao, idProm, detalhes) {
    db.collection("logs_estrelas").add({
        timestamp: new Date().getTime(),
        data_hora: new Date().toLocaleString('pt-BR'),
        autor: usuarioLogadoNick,
        beneficiado: bene,
        acao: acao,
        id_promocao: idProm || '-',
        detalhes: detalhes
...
function escutarConfigDashboard() {
    db.collection("sistema").doc("config_metas").onSnapshot((doc) => {
        if (doc.exists) {
            let d = doc.data();
            eventoAtivo = d.eventoAtivo || false;
            eventoMult = parseInt(d.eventoMult) || 1;
            pontosExtrasMap = d.pontosExtras || {};
                ...
                if (dashboardEventosData.length === 0) {
                    ...
                    dashboardEventosData.forEach((ev, i) => {
                        ...
                        if (ev.premiosTexto || ev.hc || ev.moedas) {
                            ...
                            if (ev.hc || ev.moedas) {
                                let wHC = layoutConfig[`img-premio-hc-${i}`] ? layoutConfig[`img-premio-hc-${i}`].width : '40px';
                                let wM = layoutConfig[`img-premio-moedas-${i}`] ? layoutConfig[`img-premio-moedas-${i}`].width : '40px';
...
function aplicarPosicoes() {
    for (let id in layoutConfig) {
        if (id === 'sponsorInner') continue;
        let el = document.getElementById(id);
        if (el) {
            if (layoutConfig[id].left) el.style.left = layoutConfig[id].left;
            if (layoutConfig[id].top) el.style.top = layoutConfig[id].top;
            if (layoutConfig[id].width) el.style.width = layoutConfig[id].width;
        }
    }
    aplicarSponsorInner();
}

function aplicarSponsorInner() {
    let conf = layoutConfig['sponsorInner'] || { width: '60px', left: '0px', top: '-10px' };
...
window.toggleEditMode = function() {
    ...
    if (isEditMode) {
        ...
        document.getElementById('nick-1').style.display = "block";
        document.getElementById('nick-2').innerText = "NICK 2";
        document.getElementById('nick-2').style.display = "block";
        ...
        if (!document.getElementById('select-membro').value) {
            areaDet.style.display = 'none';
            areaDet.style.opacity = '0';
        } else {
            window.renderMemberDetails();
        }
        processarPodio();
...
function setupPrizesResizable() {
    document.querySelectorAll('.resizable-prize').forEach(img => {
        ...
        img.addEventListener('wheel', (e) => {
            ...
            layoutConfig[img.id] = layoutConfig[img.id] || {};
            layoutConfig[img.id].width = w + 'px';
        }, { passive: false });
    });
}
    ...
    document.querySelectorAll('.draggable-item').forEach(el => {
        ...
        el.addEventListener('mousedown', (e) => {
            ...
            const onMouseUp = () => {
                ...
                layoutConfig[el.id] = layoutConfig[el.id] || {};
        ...
        el.addEventListener('wheel', (e) => {
            ...
            el.style.width = w + 'px';
            layoutConfig[el.id] = layoutConfig[el.id] || {};
            layoutConfig[el.id].width = el.style.width;
        }, { passive: false });
    });
}
...
function escutarMetasDoFirebase() {
    db.collection("sistema").doc("metas").onSnapshot((doc) => {
        if (doc.exists && doc.data().dados) {
            let rows = [];
            try {
                rows = JSON.parse(doc.data().dados);
            } catch (e) {
                return;
            }
            ...
            for (let i = 20; i < rows.length; i++) {
                ...
                membrosDataArray.push({
                    cargo: rows[i][2] || '',
                    nick: rows[i][3].trim(),
                    convite: parseInt(rows[i][5]) || 0,
                    ppp: parseInt(rows[i][6]) || 0,
                    rels: parseInt(rows[i][7]) || 0,
                    relcg: parseInt(rows[i][9]) || 0,
                    avisos: parseInt(rows[i][10]) || 0,
                    total_base: parseInt(rows[i][11]) || 0,
    ...
    document.querySelectorAll('.sponsor-avatar img').forEach(img => {
        ...
        img.addEventListener('mousedown', (e) => {
            ...
            const onMv = (mEv) => {
                ...
                layoutConfig['sponsorInner'].left = (sL + (mEv.clientX - sX)) + 'px';
...
function getPontuacaoFinal(m) {
    return (m.total_base * eventoMult) + (pontosExtrasMap[m.nick] || 0);
}
...
function processarPodio() {
    ...
    let a2 = document.getElementById('avatar-2');
    let m2 = document.getElementById('medal-2');
    let n2 = document.getElementById('nick-2');
    let ae1 = document.getElementById('avatar-empate-1');
    let te1 = document.getElementById('txt-empate-1');
    let ae2 = document.getElementById('avatar-empate-2');
    let te2 = document.getElementById('txt-empate-2');
    ...
    let top = [...membrosDataArray].sort((a, b) => getPontuacaoFinal(b) - getPontuacaoFinal(a));
...
window.renderMemberDetails = function() {
    ...
    document.getElementById('det-relcg').innerText = m.relcg * eventoMult;
    document.getElementById('det-avisos').innerText = m.avisos * eventoMult;
