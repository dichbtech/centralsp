...
async function verificarAcessoBD(email) {
    try {
        ...
        let userPlan = null;
        for(let key in planilhaAcessos) { if(key.toLowerCase().trim() === authEmail) { userPlan = planilhaAcessos[key]; break; } }
...
function renderTabelaAcessos() { 
    ...
    acessosData.forEach(item => { tbody.appendChild(criarRowAcesso(item, 'manual')); rendered.add(item.email.toLowerCase()); });
...
window.salvarAcessos = function() { 
    ...
    acessosData.forEach(ac => { batch.delete(db.collection("acessos").doc(ac.email)); }); 
    rows.forEach(r => { 
